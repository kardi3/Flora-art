...
class Menu_Service_Menu extends MF_Service_ServiceAbstract {
    ...
    public function fetchMainMenu($limit = 10, $hydrationMode = Doctrine_Core::HYDRATE_RECORD){
        $q = $this->menuItemTable->getMenuMainItemsQuery(1);
        $q->limit($limit);
    ...
    public function fetchMenu($limit = 10, $hydrationMode = Doctrine_Core::HYDRATE_RECORD){
        $q = $this->menuItemTable->getMenuItemQuery();
        $q->limit($limit);
        $banners = $q->execute(array(), $hydrationMode);
        //$this->incrementViews($banners);
        return $banners;
	...
	if(strlen($menuItemArray['target_type']))
        $menuItemArray['target'] = $menuItemArray['route'].'_'.$menuItemArray['target_type'];
	else
	...
	$menuItem['route'] = $data['target'];
        foreach($data['translations'] as $language => $translation) {
            $menuItem->Translation[$language]->title = $translation['title'];
	    $menuItem->Translation[$language]->slug = MF_Text::createUniqueTableSlug('Menu_Model_Doctrine_MenuItemTranslation', $data['translations'][$language]['title'], $menuItem->get('id'));
	...
	if(strpos($data['target'],'domain-i18n:page')!==false){
	    $val = explode('_',$data['target']);
	    $menuItem->set('route',$val[0]);
	    $menuItem->set('target_type',$val[1]);
	}
        $menuItem->save();
