...
class User_Service_User extends MF_Service_ServiceAbstract {
    ...
    public function saveUserFromArray($values,$user_id=null) {
        foreach($values as $key => $value) {
            if(!is_array($value) && strlen($value) == 0) {
                $values[$key] = NULL;
            }
        ...
        if(!$user = $this->userTable->getProxy($user_id)) {
            $user = $this->userTable->getRecord();
        }
...
//        
    ...
    public function sendRegistrationMail(User_Model_User_Interface $user, Zend_Mail $mail, Zend_View_Interface $view, $partial = 'email/register.phtml') {
        $token = $user->getToken();//                      
        $mail->setBodyHtml(
            $view->partial($partial,'user', array('token' => $token))
        );
        $mail->send(); 
    }
    ...
    public function sendRecoverMail(User_Model_User_Interface $user, Zend_Mail $mail, Zend_View_Interface $view, $partial = 'email/recover-password.phtml') {
        $token = $user->getToken();  
        $mail->setBodyHtml(
            $view->partial($partial, array('token' => $token))
        );
