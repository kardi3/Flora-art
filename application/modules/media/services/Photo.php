...
class Media_Service_Photo extends MF_Service_ServiceAbstract {
    ...
    public function findPhotos($slug, $hydrationMode = Doctrine_Core::HYDRATE_RECORD) { 
        $q = $this->photoTable->createQuery('p');
	$q->addSelect('p.*');
        $q->andWhere('p.title like ?', "%".$slug."%");
        $q->orderBy('p.title ASC');
        return $q->execute(array(), $hydrationMode);
    }
    
    public function setPhotoRow($data)
    {
    ...
    public function crop($filePath, $destPath, $x, $y, $x2, $y2, $destWidth = null, $destHeight = null) {
        if(file_exists($filePath)) {
            ...
            imagealphablending($sourceGdImage, true);
