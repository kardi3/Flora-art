 ...
 * @license http://www.opensource.org/licenses/bsd-license.php  New BSD License
 */
class MF_Application_Resource_Doctrine extends Zend_Application_Resource_ResourceAbstract
{
    ...
    public function getDoctrine() {
        if (null === self::$_doctrine) {
            ...
            foreach($options['autoload'] as $namespace => $enabled) {
                if($enabled) {
                    $this->getBootstrap()->getApplication()->getAutoloader()->pushAutoloader(array('Doctrine_Core', $namespace));
                }
            }
            ...
            if(isset($options['attribute'])) {
                foreach($options['attribute'] as $name => $value) {
                    ...
                    $manager->setAttribute(constant(sprintf('%s::%s', 'Doctrine_Core', $name)), $value);
                }
            }
            if(isset($options['models_path']) && $options['autoload']['modelsAutoload'] === true) {
                Doctrine_Core::loadModels($options['models_path']);
            }

            $manager->openConnection($options['connection_string'], $options['connection_name']);
            $manager->getCurrentConnection()->setCharset('utf8');

            // extensions
            if(isset($options['extensions_path'])) {
                Doctrine_Core::setExtensionsPath($options['extensions_path']);
    ...
    public static function loadCacheDriver(array $options) {
        if(empty($options) || (isset($options['disable_caching']) && (bool) $options['disable_caching'] === true)) {
            return null;
        }
        unset($options['disable_caching']);
